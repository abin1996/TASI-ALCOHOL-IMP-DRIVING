...
#Adjust the logging level to logging.DEBUG if you want to see the debug logs
logging.basicConfig(level=logging.INFO, format='%(asctime)s %(levelname)s %(message)s', filename=log_filename, filemode='w')


#DATA CLASSIFICATION is the process of categorizing the data into subcategories based on the event signal
...
#IMAGE EXTRACTION is the process of extracting images from the bag files 
...
#IMAGE SYNC is the process of syncing the images from the different cameras
...
#CAN EXTRACTION is the process of extracting the CAN data for the subcategories
...
#GPS EXTRACTION is the process of extracting the GPS data for the subcategories 
...
#VIDEO EXTRACTION USING FRAMES is the process of extracting the video from the images
...
# PERSPECTIVE TRANSFORMATION is the process of transforming the images to a top down view. This is done to calculate the lane deviation from the center of the lane
def perform_lane_deviation_calculation(subject_id, alcohol_session_name, data_classification_folder_type, sub_categories_to_classify, target_folder_parent_path, lane_deviation_parameters,mat_hor_len):
    ...
    for sub_category in sub_categories_to_classify:
        ...
        for sub_category_run_number in range(1, sub_category_runs + 1):
            ...
            #Delete the transformed images folder since it is not needed
            if os.path.exists(camera_output_folder):
...
#COMBINE LANE DEVIATION OUTPUT FROM SIDE CAMERAS is the process of combining the lane deviation output from the left and right cameras to get the lane deviation
...
if __name__ == '__main__':
    # The config json file is stored in the config folder. Use the path to the config file as the first argument.
    # More config json files can be made with similar structure to the one in the config folder.
